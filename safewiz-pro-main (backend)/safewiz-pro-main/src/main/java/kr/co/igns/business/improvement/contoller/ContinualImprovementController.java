...
import kr.co.igns.business.improvement.service.ContinualImprovementService;
...
public class ContinualImprovementController {
	...
	private final ContinualImprovementService continualImprovementService;

	
	@Operation(summary = "[지속적 개선 타이틀 메뉴 조회]", description = "지속적 개선 타이틀 메뉴를 조회한다")
	@GetMapping(value = "/safewizpro/improvement/getContinualImprovementTitleMenu")
	public ResponseEntity<SingleResponseDto<HashMap<String, Object>>> getContinualImprovementTitleMenu(
			@ModelAttribute @Valid SpSearchVO searchVo) throws Exception {
		List<ContinualImprovementVO> list = continualImprovementService.getContinualImprovementTitleMenu(searchVo);
