...
def _capture_frames(rtsp_url: str, count: int = SNAPSHOT_FRAMES) -> list[np.ndarray]:
    """RTSP에서 count개의 프레임을 캡처하고 반환한다. 연결 실패 시 3회 재시도한다."""
    cap = None
    for attempt in range(3):  # 총 3회 시도
        cap = cv2.VideoCapture(rtsp_url, cv2.CAP_FFMPEG)
        if cap.isOpened():
            break
        print(f"[Progress] RTSP open attempt {attempt + 1}/3 failed: {rtsp_url}")
    ...
    if cap is None:
        print(f"[Progress] RTSP open failed after 3 attempts: {rtsp_url}")
        return []
