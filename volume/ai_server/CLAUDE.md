...
└── service/                         # 비즈니스 도메인 로직
    ...
    │   └── routes.py
    ├── telegram/                    # 텔레그램 알림 관리
    │   ├── model.py                 # TelegramManager (tb_telegram_managers)
    │   ├── schema.py                # 매니저 등록/삭제 요청 스키마
    │   ├── repository.py            # 매니저 CRUD + 알림 대상 조회
    │   ├── service.py               # 텔레그램 알림 전송 + 매니저 관리
    │   └── routes.py                # /cctv/manager_crud/* 엔드포인트
    ├── remote/                      # 원격 제어
    │   ├── service.py
    │   └── routes.py
    └── save/                        # 이미지 저장 관리
...
MediaMTX (미디어 서버)        # 스트림 중계, 자동 재연결, 프로토콜 변환
    ...
    │                            #   → send_telegram_alert() → 텔레그램 푸시 알림
    │
    └─► 프론트엔드 (WebRTC/HLS)  # 실시간 영상 시청
```

**roi_arr, detection_is_run, pose_is_run** — `register_camera` 시 DB 조회 후 프로세스 인자로 전달
- `detection_is_run`, `pose_is_run`은 `tb_camera_roi`의 `is_run` 필드에서 model_nm(Detection/Pose)별로 조회
...
| E004 | 위험물 접근 | SAFETY (person + forklift/hoist, 안전 격자 기반 감지) |
...
### 텔레그램 매니저 관리 (`/cctv/manager_crud`)

| 메서드 | 경로 | 설명 |
|--------|------|------|
| GET | `/cctv/manager_crud/lists` | 텔레그램 매니저 목록 조회 |
| POST | `/cctv/manager_crud/manager` | 매니저 등록/수정 (`list[TelegramManagerSave]`) |
| DELETE | `/cctv/manager_crud/manager` | 매니저 삭제 (`list[TelegramManagerDelete]`) |
| GET | `/cctv/manager_crud/run_alarm/{composite_key}` | 개별 알림 활성화 |
| GET | `/cctv/manager_crud/stop_alarm/{composite_key}` | 개별 알림 비활성화 |
| GET | `/cctv/manager_crud/run_all` | 전체 알림 일괄 활성화 |

`composite_key` 형식: `{monitoring_grp_id}_{chat_id}` — `rsplit("_", 1)`로 파싱
...
| 테이블 | 도메인 | 용도 |
|--------|--------|------|
| `tb_camera` | cctv / progress | 카메라 기본 정보 + 공정률 설정 (camera_id, rtsp_addr, camera_nm, jit_only, sort_direction) |
| `tb_camera_roi` | roi | ROI 폴리곤 좌표 + AI 모델 스위치 (is_run per model_nm: Detection/Pose) |
| `tb_camera_event_hist` | safety | 안전 이벤트 이력 |
| `tb_telegram_managers` | telegram | 텔레그램 알림 매니저 (chat_id, is_alarm) |
