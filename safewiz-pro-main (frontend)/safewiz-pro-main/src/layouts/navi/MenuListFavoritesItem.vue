<template>
    <li :class="{ collapsed: isCollapsed, active: isActive }">
        <a @click="navigate" class="df aic" :title="menuItem.menuNm">
            <!-- <i class="menu-icon"><img v-if="menuItem.menuIcon" :src="`/assets/img/common/${menuItem.menuIcon}.svg`"
                    alt="" /></i> -->
            <span class="ellipsis maxw100p">{{ menuItem.menuNm }}</span>
            <!-- @click.stop.prevent="toggleFavoritesOption(menu.menuId) -->
            <button type="button" class="w20px h20px ml8px cAAAAAA hover-cFF3534 fs0" @click.stop.prevent="deleteBtnClick">
...
const deleteBtnClick = () => {
    emit('deleteItem', props.menuItem.menuId);
};
</script>
