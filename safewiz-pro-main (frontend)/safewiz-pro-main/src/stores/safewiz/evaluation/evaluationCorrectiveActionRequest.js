...
const { openLoading, endLoading,reactive, alertMsg, ref, toastPopup, confirmMsg, getCompId, getCurrentDate, t, baseDownload, formatDateForDB, formatDate, saveSignAsync  } = BaseView();
...
//API
import {getCorrectiveActionRequest, getCorrectiveActionRequestDetail, saveCorrectiveActionRequest, updateCorrectiveActionRequest, deleteCorrectiveActionRequest, deleteCorrectiveActionRequests, printCorrectiveActionRequests} from '@/stores/safewiz/evaluation/api/evaluationCorrectiveActionRequestApi.js'
import _ from 'lodash';

export const useEvaluationCorrectiveActionRequestStore = defineStore('evaluationCorrectiveActionRequest', () => {
  ...
  const initInputForm = () => {
    ...
    const writeDt = formatDate(monitoringMngStore.searchParam.searchText  + month + day)
    inputForm.value = {
    docType: 'CAR',
  ...
  const closeOrgnAction = e => {
    ...
    if (e && e.length) {
      ...
      // signature2.value.setHrInfo(1,refinedItems)
  ...
  const initSearchParam = () => {
    searchParam.value = {
      writeYear: monitoringMngStore.searchParam.searchText,
      compId: getCompId(),
      content : ''
    };
    checkedList.value = [];
  ...
  const btnAdd = () => {
    ...
    router.push({
        name: 'EvaluationCorrectiveActionRequestDetail'
    });
  };
  ...
  const deleteCorrectiveActionRequestFunction  = (type, location, param) => {
    if(location == 'D'){ //상세
      ...
      if(type==1){ //다중 삭제
        openLoading();
        return deleteCorrectiveActionRequests(param, true).then(res => {
          btnSearch(true)
        }).catch(err => {
          alertMsg('error', '에러', err)
        }).finally(()=>{
