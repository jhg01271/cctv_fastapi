...
import { getJobAssign, getJobAssignDetail, saveJobAssign, deleteJobAssign, deleteJobAssigns} from "@/stores/safewiz/participation/api/hseJobAssignmentApi"
import { useButtonListStore } from '@/stores/buttonList';
const layoutStore = useButtonListStore();
import _ from 'lodash';

export const useHseJobAssignmentStore = defineStore("HseJobAssignment", () => {
  ...
  const detailHrId = ref(null)
  const inputForm = ref({});
  const initInputForm = (writeYear) => {
    inputForm.value = {
      cmd: 'I',
  ...
  //HSE 업무분장 상세조회
  const getJobAssignDetailList = (notify) => {
    searchParam.value.writeYear = searchParam.value.writeYear.slice(0, 4);
    searchParam.value.hrId = detailHrId.value
    return getJobAssignDetail(searchParam.value, notify)
      .then(res => {
        searchParam.value.hrId = null
        if (res && res.success) {
          if(res.list) {
            inputForm.value = res.list
          } else {
            initInputForm(searchParam.value.writeYear)
          }
 ...
 const deleteJobAssignLists = (data) => {
      ...
      saveJobAssign(data, true)
          .then(res => {
            if (res.success) {
              ...
              detailHrId.value = res.result.hrId
              getJobAssignDetailList(false);
  ...
  return {
    initInputForm, inputForm, originData,
    jobAssignList, buttonList,
    searchParam, toggleUseYn,
    checkedList, initData, searchTerm, dataFilterList, applyFilter,
    currentDate,detailHrId,
    //라우터
    btnAdd, goBack, goObject,
    //버튼
    btnDelete, btnSave,
    //api
    getJobAssignList, getJobAssignDetailList,
  }
})
