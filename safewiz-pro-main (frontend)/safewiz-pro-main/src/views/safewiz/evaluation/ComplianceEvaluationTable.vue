    ...
    <div id="form" class="contents df fdc">
        ...
        <div class="form ui mb2-2rem">
            <div class="row flex gutters1rem">
                <div class="grid12-3 es-grid12-12">
                    <input v-model="monitoringMngStore.searchParam.searchText" v-input type="text" v-calendar="'yyyy'" year class="datepicker w100p radius" @input="complianceEvaluationStore.getLegalEvaluationList(true)" />
                </div>
                <div class="grid12-9 es-grid12-12">
                    <div class="df">
                        <input v-model="complianceEvaluationStore.searchText" v-input type="text" class="radius w100p search" placeholder="검색어를 입력하세요" @keyup.enter="complianceEvaluationStore.dataFilter" />
                        <button type="submit" class="shrink0" @click.stop="complianceEvaluationStore.dataFilter">
        ...
        >
            ...
            <div class="accordion df fdc rg8px">
                <!-- 1 -->
                <div v-for="(segment, index) in complianceEvaluationStore.segments" :key="index" class="list">
                    <button type="button" class="df jcsb aic" :id="`accordion-btn_${index}`" @click="complianceEvaluationStore.toggleAccordion" :class="{ active: complianceEvaluationStore.activeSegments[index] }">
                        <em>{{ segment.month }}</em>
                        ...
                        <div class="pa2-2rem">
                            <div class="row flex gutters2rem" :key="segment.data">
                                ...
                                <template v-for="(data, idx) in segment.data" :key="`card_${idx}`">
                                    <i-card :v-model="data.checked" :modelValue="data.checked" :data="data" :type="'basic'" :title="data.legalEvaluationNm" @handle="handleEvent" @editor="btnDetail" :disabled="data.useYn == 'N'" :useApprovalStatus="true" :approvalStatus="data.approvalStatus"></i-card>
                                </template>
                                ...
                                <div class="grid12-4 ul-grid12-6 lg-grid12-12">
                                    <div class="card h100p df aic jcc cp" @click="addAction">
                                        <div class="tac">
                                            ...
                                            <span class="db fs2rem c999999">{{ t('card_addLeaglComplianceEvaluationTable') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                ...
                <!-- 리스트가 없을시 버튼만 나오도록 -->
                <div v-if="complianceEvaluationStore.segments.length < 1">
                    <div class="grid12-4 ul-grid12-6 lg-grid12-12">
                        <div class="card h100p df aic jcc cp" @click="addAction">
                            <div class="tac">
                                <svg width="46" height="46" viewBox="0 0 46 46" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const { ref, onMounted, t, formatDate, getCurrentDate, watch, btnBack, btnSearch, btnAdd, btnDelete, btnDownload, toastPopup, confirmMsg, getCompId, goRouter, alertMsg } = BaseView();
//-----------------------------------------------
// [스토어]
...
layoutStore.useBtnList = ['btnBack', 'btnAdd', 'btnSearch', 'btnDelete', 'btnDownload'];
...
const addAction = () => {
    // 데이터 초기화
    complianceEvaluationStore.initInputForm();
    // 추가 플래그
    complianceEvaluationStore.inputForm.cmd = 'I';
    router.push({
        path: 'ComplianceEvaluationTableDetail'
    });
};

/* 조회 */
btnSearch(() => {
    complianceEvaluationStore.getLegalEvaluationList(true);
});
...
btnDelete(() => {
    let checkedList = [];
    // 체크한 값이 없을경우 메세지 호출
    complianceEvaluationStore.segments.forEach(el =>
        el.data.forEach(id => {
            if (id.checked) {
                const param = {
                    writeYear: id.writeYear,
                    docNo: id.docNo,
                    legalEvaluationNm: id.legalEvaluationNm,
                    compId: getCompId(),
                    docType: id.docType
                };
    ...
    confirmMsg('warning', t('msgDelete'), ``, {
        fun: complianceEvaluationStore.deleteLegalEvaluation,
        param: checkedList
    });
});
...
btnDownload(() => {
    ...
    const searchVO = {
        ...
        checkedObjList: checkedObjList,
        checkedList: checkedList
    };

    complianceEvaluationStore.btnDownload(searchVO);
});
...
onMounted(() => {
    if (!complianceEvaluationStore.writeYear) {
        complianceEvaluationStore.searchParam.writeYear = getCurrentDate().substring(0, 4);
    } else {
        complianceEvaluationStore.searchParam.writeYear = complianceEvaluationStore.writeYear;
    }
    complianceEvaluationStore.getLegalEvaluationList();
});
...
const btnDetail = e => {
    ...
    goRouter('ComplianceEvaluationTableDetail', param);
};

const handleEvent = e => {
    //체크된 데이터를 checkedList 에 담음
    if (e.checked) {
        complianceEvaluationStore.checkedList.push(e);
    } else {
        complianceEvaluationStore.checkedList = complianceEvaluationStore.checkedList.filter(item => item.writeYear + item.docType + item.docNo !== e.writeYear + e.docType + e.docNo);
    }
};
</script>
