                                    ...
                                    <div class="grid12-6 es-grid12-12">
                                        <div class="card h100p df aic jcc cp" @click="addAction">
                                            <div class="tac">
                                                ...
                                                <span class="db mt1rem fs2rem c999999">{{ t('card_safetyAndHealthOrganChart') }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
...
import { confirmOrganizationChart, deleteOrganizationChart, getOrganizationChart, getOrganizationChartDetail } from '@/stores/safewiz/participation/api/safetyOrgChartApi';
const { openLoading, endLoading, btnBack, onMounted, router, ref, t, btnAdd, getCompId, btnDelete, btnSearch, confirmMsg, alertMsg, formatDate } = BaseView();
...
btnBack(() => {
    router.push('/SafetyAndHealthOrganChart');
});
...
const addAction = () => {
    ...
    router.push('/SafetyAndHealthOrganChartDetail');
};
...
const confirmAction = item => {
    openLoading();
    confirmOrganizationChart({ chartId: item.chartId, confirmYn: item.confirmYn }, true)
        .then(() => {
            init();
        })
        .catch(() => {
...
const confirmedDegree = computed(() => {
    return degreeList.value.filter(el => {
...
const init = filter => {
    ...
    getOrganizationChart(searchParam.value, true).then(res => {
        if (filter) {
            degreeList.value = res.list.filter(el => {
                return el.chartNm?.includes(filterValue.value) || formatDate(el.confirmDt)?.includes(filterValue.value) || el.orgCount?.includes(filterValue.value) || formatDate(el.createdAt)?.includes(filterValue.value);
            });
        } else {
            degreeList.value = res.list;
        }
        degreeList.value.forEach(el => {
            el.detail = {
...
buttonListStore.useBtnList = ['btnAdd', 'btnSearch', 'btnDelete', 'btnBack'];
// 아코디언 토글
