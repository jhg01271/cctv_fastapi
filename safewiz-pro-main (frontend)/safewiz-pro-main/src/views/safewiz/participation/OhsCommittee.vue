...
const { goRouter, ref, t, onMounted, btnBack, btnAdd, btnDelete, btnDownload, alertMsg, confirmMsg, btnSearch, formatDate, endLoading, openLoading } = BaseView();
...
const filterByValue = () => {
    ...
    list.value = originalList.value.filter(item => {
        return (
            ...
            (item.companyCount?.toString().includes(search)) ||
            (item.count?.toString().includes(search)) ||
            (item.percent?.toString().includes(search)) ||
            ((item.commDiv == 'E' ? '정기' : '수시').toLowerCase().includes(search))
        );
    });
...
const init = filter => {
    openLoading();
    getParticipation('OHC', true)
        .then(res => {
            segments.value = [partnerCommitteeStore.writeYear];
            if (filter) {
                res.list = res.list.filter(el => {
                    return formatDate(el.commDt).includes(filterValue.value) || el.commNm?.includes(filterValue.value) || `${el.companyCount}/${el.employeeCount} 명`.includes(filterValue.value) || (el.commDiv == 'E' ? '정기' : '수시').includes(filterValue.value) || `${el.count}건`.includes(filterValue.value) || `${el.percent}%`.includes(filterValue.value);
                });
            } else {
                list.value = res.list;
            }
            list.value = res.list.map(el => {
                return {
                    ...
                    detail: {
                        회의명: el.commNm,
                        '참석(사측/근로자)': `${el.companyCount}/${el.employeeCount}`,
                        구분: el.commDiv == 'E' ? '정기' : '수시',
...
const btnDetail = item => {
    ...
    goRouter('OhsCommitteeDetail', param);
};
btnAdd(() => {
    addAction();
});
...
const downloadAction = param => {
    partnerCommitteeStore.downloadOhsReport(param);
};
// 아코디언 토글
gsap.registerPlugin(CustomEase);
CustomEase.create('customEase', 'M0,0 C0.9,0 0.2,1 1,1');

const segments = ref([partnerCommitteeStore.writeYear]);
