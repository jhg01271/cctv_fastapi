        ...
        <div class="control-field ui form mb2-2rem">
            <div class="row flex gutters1rem">
                <div class="grid12-3 es-grid12-12">
                    <input v-input type="text" v-calendar="'yyyy'" year v-model="hseJobAssignmentStore.searchParam.writeYear" @input="hseJobAssignmentStore.getJobAssignList(true)" class="datepicker w20rem radius es-w100p" />
                </div>
                <div class="grid12-9 es-grid12-12">
                    <div class="df bcffffff">
                        <input v-input type="text" class="radius w100p search" :placeholder="t('placeHolderSearch')" v-model="hseJobAssignmentStore.searchTerm" @keyup.enter="handleSearch" />
        ...
        >
            <div class="oh">
                <div class="row flex gutters2rem">
                    <template v-for="jobAssign in hseJobAssignmentStore.dataFilterList" :key="jobAssign.jobAssignId">
                    ...
                    <div class="grid12-4 ul-grid12-6 lg-grid12-12">
                        <div class="card h100p df aic jcc cp" @click="hseJobAssignmentStore.btnAdd()">
                            <div class="tac">
...
// 조회 버튼
btnSearch(() => hseJobAssignmentStore.getJobAssignList(true));

// 추가 버튼
btnAdd(() => hseJobAssignmentStore.btnAdd());

// 삭제 버튼
btnDelete(() => hseJobAssignmentStore.btnDelete());

// 초기 데이터 로딩
onMounted(async () => {
    if (!hseJobAssignmentStore.searchParam.writeYear) {
        //현재년도 세팅
        hseJobAssignmentStore.searchParam.writeYear = hseJobAssignmentStore.currentDate.slice(0, 4);
    }
    hseJobAssignmentStore.getJobAssignList(true);
});
...
const handleEvent = e => {
    if (e.checked) {
        ...
        hseJobAssignmentStore.checkedList.push(e);
    } else {
        // e의 writeYear, docNo, docType 값이 일치하는 데이터를 제외한 리스트 생성
        hseJobAssignmentStore.checkedList = hseJobAssignmentStore.checkedList.filter(item => item.writeYear !== e.writeYear || item.docNo !== e.docNo || item.docType !== e.docType);
    }
};
// 검색 핸들러
const handleSearch = () => hseJobAssignmentStore.applyFilter();
...
const btnDetail = async e => {
    ...
    // hseJobAssignmentStore.searchParam.writeYear = e.writeYear;
    hseJobAssignmentStore.searchParam.docNo = e.docNo;
    hseJobAssignmentStore.searchParam.docType = e.docType;
    hseJobAssignmentStore.searchParam.hrId = e.hrId;
    hseJobAssignmentStore.detailHrId = e.hrId;
    hseJobAssignmentStore.searchParam.userId = e.userId;
    ...
    router.push({ name: 'HseJobAssignmentDetail' });
    hseJobAssignmentStore.searchParam.hrId = null;
    hseJobAssignmentStore.searchParam.userId = null;
};
</script>
