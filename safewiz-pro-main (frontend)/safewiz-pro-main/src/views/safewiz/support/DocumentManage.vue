    ...
    <div id="form" class="contents df fdc">
        ...
        <div class="control-field ui form mb2-2rem">
            <div class="df bcffffff">
                <input v-model="docMngStore.searchText" v-input type="text" class="radius w100p search" :placeholder="t('placeHolderSearch')" @keyup.enter="docMngStore.dataFilter" />
                <button type="submit" class="shrink0" @click.stop="docMngStore.dataFilter">
        ...
        >
            ...
            <div class="accordion df fdc rg8px">
                <!-- 1 -->
                <div v-for="(segment, index) in docMngStore.segments" :key="index" class="list">
                    <button type="button" class="df jcsb aic" :id="`accordion-btn_${index}`" @click="docMngStore.toggleAccordion" :class="{ active: docMngStore.activeSegments[index] }">
                        <em>{{ segment.year }}년도</em>
                        ...
                        <div class="pa2-2rem">
                            <div class="row flex gutters2rem">
                                ...
                                <template v-for="(data, idx) in segment.data" :key="`card_${idx}`">
                                    <i-card :v-model="data.checked" :modelValue="data.checked" :data="data" :type="'basic'" :title="data.manageDocNm" @handle="handleEvent" @editor="btnDetail" :disabled="data.useYn == 'N'"></i-card>
                                </template>
                                <!-- 추가 -->
                                <div class="grid12-4 ul-grid12-6 lg-grid12-12">
                                    <div class="card h100p df aic jcc cp" @click="docMngStore.btnAdd()">
                                        <div class="tac">
                                            ...
                                            <span class="db mt1rem fs2rem c999999">{{ t('card_addDocument') }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
const { ref, onMounted, t, formatDate, watch, btnSearch, btnAdd, btnDelete, toastPopup, confirmMsg, getCompId } = BaseView();
...
const btnNew = () => {
    // docMngStore.goInsert();
};

btnDelete(() => {
    docMngStore.btnDelete();
});
...
const btnDetail = e => {
    ...
    docMngStore.goDetail(param);
};

const handleEvent = e => {
    //체크된 데이터를 checkedList 에 담음
    if (e.checked) {
        docMngStore.checkedList.push(e);
    } else {
        docMngStore.checkedList = docMngStore.checkedList.filter(item => item.writeYear + item.docType + item.docNo !== e.writeYear + e.docType + e.docNo);
    }
};
</script>
