<template>
    <div id="popup" class="form ui">
        <h3>점검사항 관리</h3>
        <div class="form ui field df aic">
            ...
            <div class="row flex gutters1rem">
                <div class="grid10-3 es-grid10-10">
                    <div class="field df jcsb">
                        <label for>점검사항 유형</label>
                    </div>
                </div>
                <div class="grid10-7 es-dn">
                    <div class="field">
                        <label for>점검사항 유형별 점검사항</label>
                    </div>
                </div>
                <div class="grid10-3 es-grid10-10">
                    <div class="pr bd1pxsolidE1E6ED br4px h30vh">
                        ...
                        >
                            <div class="df fdc rg1rem">
                                <button v-for="(eType, index) in filteredTypeList" :key="eType.inspectionId" class="btn br4px" v-button @click="typeClick(eType)" :id="`type_${index}`">{{ eType.inspectionNm }}</button>
                            </div>
                        </OverlayScrollbarsComponent>
                ...
                <div v-if="filteredTypeList.length !== 0" class="grid10-7 es-grid10-10">
                    <div class="h100p pa2rem bd1pxsolidE1E6ED br4px oh es-pa1rem">
                        <div class="row flex gutters1rem">
                            <div class="grid10-6 es-grid10-10">
                                <div class="field">
                                    <label for="inspectionNm" required> <span> 점검사항 유형명 </span> </label>
                                    <input type="text" class="br4px" v-input v-model="selectedType.inspectionNm" placeholder="점검사항 유형명을 입력하세요." @change="selectedType.checked = true" id="inspectionNm" required />
                                </div>
                            </div>
                            <div class="grid10-2 es-grid10-5">
                                <div class="field">
                                    <label for>순서</label>
                                    <input v-input type="number" class="br4px tac" v-model="selectedType.ordSeq" min="0" max="99" placeholder="0" @change="selectedType.checked = true" />
                                </div>
                            </div>
                            <div class="grid10-2 es-grid10-5">
                                <div class="field">
                                        ...
                                        <table class="tac es-minw50rem">
                                            ...
                                            <tbody>
                                                <tr v-for="(role, index) in selectedType?.itemList" :key="role.type + '_' + role.role" :id="`role_${index}`">
                                                    ...
                                                    <td>
                                                        <input type="text" class="br4px" v-input v-model="role.inspectionItemNm" @change="detailValueChanged(role)" />
                                                    </td>
                                                    <td>
        ...
        <div class="df jcsb">
            <button type="button" class="btn delete w7rem radius" v-button @click="btnDelete">
                <span>삭제</span>
            </button>
            <div class="df gap8px">
...
import { getSafetyWorkChecklist, getSafetyWorkChecklistDetail, saveSafetyWorkChecklist, deleteSafetyWorkChecklistDetail } from '@/stores/safewiz/impl/api/permitToWorkApi.js';
...
const btnSearch = (notify = false) => {
    searchTerm.value = '';
    getSafetyWorkChecklist({ compId: compId }, notify).then(res => {
...
const btnAddDetail = () => {
    if (selectedType.value.itemList) {
        selectedType.value.itemList.push({
            ...
            compId: getCompId(),
            cmd: 'I',
            inspectionId: selectedType.value.inspectionId,
            inspectionItemNm: '',
            ordSeq: 99,
            useYn: 'Y'
        });
...
const btnSaveDetail = async () => {
    ...
    if (isValid) {
        if (selectedType.value.checked) {
            confirmMsg('info', `[${selectedType.value.inspectionNm}]\n${t('msgSave')}`, null, { fun: saveAction, param: true });
        } else {
...
const saveAction = notify => {
    ...
    saveSafetyWorkChecklist(saveData, notify)
        .then(res => {
...
const deleteAction = param => {
    deleteSafetyWorkChecklistDetail(param, true)
        .then(res => {
            selectedType.value.checked = false;
            selectedType.value.itemList.forEach(el => (el.checked = false));
        })
...
const applyFilter = () => {
    ...
    filteredTypeList.value.forEach(el => {
        el.itemList = el.itemList.filter(role => role.inspectionItemNm.toLowerCase().includes(searchTerm.value.toLowerCase()));
    });
    const filteredData = filteredTypeList.value.filter(item => item.itemList.length !== 0 || item.inspectionNm.toLowerCase().includes(searchTerm.value.toLowerCase()));
