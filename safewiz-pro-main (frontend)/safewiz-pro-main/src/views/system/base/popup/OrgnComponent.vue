...
onMounted(async () => {
    let responses = await Promise.all([getOrganization()]);
    if (responses) {
...
// 필터 적용 함수
const applyFilter = () => {
    const filteredData = dataList.value.filter(item => item.orgnNm.toLowerCase().includes(searchTerm.value.toLowerCase()));
    // 필터링된 데이터로 그리드를 업데이트
    if (dataFilter.value) {
